...
def trainES():#hidden = 512 input = 24 out = 4
    ...
    for generation in range(n_generations):
        ...
        f =  open('rewardsES.json', 'a')
        json.dump({"gen":generation,"time":time.time()-startTime,"meanReward":sum(scores)/len(scores)},f)
        f.close()
        ...
        if generation>= 70:
            plt.show()
        if generation%5==0 or scores > bestScore:
            bestScore = scores
            BESTMODEL = population[np.argmax(scores)]
            model = AgentNN(24,512,4)
            model.loadFromTensors(W1=torch.from_numpy(BESTMODEL.network['Layer 1']),W2=torch.from_numpy(BESTMODEL.network['Layer 2']),b1=torch.from_numpy(BESTMODEL.network['Bias 1']),b2=torch.from_numpy(BESTMODEL.network['Bias 2']))
            torch.save(model.state_dict(), f'./saveES/bestAgent{generation}.pt')
        print(f'c ho messo {time.time()-a}')
